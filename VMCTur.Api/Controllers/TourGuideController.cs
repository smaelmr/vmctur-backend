...
namespace VMCTur.Api.Controllers
{
    ...
    public class TourGuideController : ApiController
    {
        ...
        public Task<HttpResponseMessage> Post(CreateGuiaModel model)
        {
            ...
            try
            {
                _service.Create(model.CompanyId, model.Name, model.Cpf, model.BondType, model.Comments);
        ...
        public Task<HttpResponseMessage> Put(UpdateGuiaModel model)
        {
            ...
            try
            {
                _service.Update(model.Id, model.EmpresaId, model.Name, model.Cpf, model.BondType, model.Comments);
                response = Request.CreateResponse(HttpStatusCode.OK, new { name = model.Name });
            }
            catch (Exception ex)
            {
