...
{
    [RoutePrefix("api/conta-pagar")]
    public class BillPayController : ApiController
    {
        private IBillPayService _service;
        ...
        [HttpPut]
        [Route("")]
        public Task<HttpResponseMessage> Put(UpdateBillPayCommand command)
        {
        ...
        [HttpPost]
        [Route("receipt")]
        public Task<HttpResponseMessage> Receipt(ReceiptBillPayCommand command)
        {
            HttpResponseMessage response = new HttpResponseMessage();
        ...
        [Route("getpaidbills")]
        [DeflateCompression]
        public Task<HttpResponseMessage> GetPaidBills(DateTime startPeriod, DateTime finishPeriod)
        {
            HttpResponseMessage response = new HttpResponseMessage();

            try
            {
                var result = _service.GetPaidBills(startPeriod, finishPeriod);
                response = Request.CreateResponse(HttpStatusCode.OK, result);
            }
            catch (Exception ex)
            {
